    ...
    public class ReflectionDocumentMapper<T> : DocumentMapperBase<T>
    {
        ...
        private void BuildFieldMap(IEnumerable<PropertyInfo> props)
        {
            foreach (var p in props)
            {
                ...
                var mappingContext = FieldMappingInfoBuilder.Build<T>(p, this.VersionProperty, this.ExternalAnalyzerProperty);

                this.FieldMapProperty.Add(mappingContext.PropertyName, mappingContext);

                if (!string.IsNullOrWhiteSpace(mappingContext.FieldName) && mappingContext.Analyzer != null)
                {
                    this.AnalyzerProperty.AddAnalyzer(mappingContext.FieldName, mappingContext.Analyzer);
                }
            }
        }
        ...
        private void BuildKeyFieldMap(Type type, IEnumerable<PropertyInfo> props)
        {
            ...
            this.KeyFieldsProperty.AddRange(keyProps.Select(kp => this.FieldMapProperty[kp.Name]));
