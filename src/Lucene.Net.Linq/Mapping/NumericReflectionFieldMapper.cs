...
namespace Lucene.Net.Linq.Mapping
{
    internal class NumericReflectionFieldMapper<T> : ReflectionFieldMapper<T>
    {
        ...
        {
            var targetType = this.PropertyInfoProperty.PropertyType;
        ...
        public override void CopyToDocument(T source, Document target)
        {
            ...
            var numericField = new NumericField(this.FieldNameProperty, precisionStep, FieldStore, true);
        ...
        {
            if (lowerBound != null && !this.PropertyInfoProperty.PropertyType.IsInstanceOfType(lowerBound))
            {
                lowerBound = ConvertToSupportedValueType(lowerBound);
            }
            if (upperBound != null && !this.PropertyInfoProperty.PropertyType.IsInstanceOfType(upperBound))
            {
                upperBound = ConvertToSupportedValueType(upperBound);
            }
            return NumericRangeUtils.CreateNumericRangeQuery(this.FieldNameProperty, (ValueType)lowerBound, (ValueType)upperBound, lowerRange, upperRange);
        }
        ...
        private object ConvertToSupportedValueType(object value)
        {
            ...
            var propertyType = this.PropertyInfoProperty.PropertyType.GetUnderlyingType();
